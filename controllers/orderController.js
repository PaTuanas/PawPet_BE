...
const calculateTotalPrice = (selectedProducts) => {
    const totalPrice = selectedProducts.reduce((total, product) => {
        if (product.productid && typeof product.productid.price === 'number' && typeof product.quantity === 'number') {
            return total + (product.productid.price * product.quantity);
        } else {
            console.warn(`Giá hoặc số lượng không hợp lệ cho sản phẩm: ${JSON.stringify(product)}`);
            return total;
        }
    }, 0);
